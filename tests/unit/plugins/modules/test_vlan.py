...
def mock_api_client():
    ...
    client.create.return_value = {"vlan_id": "res-123", "name": "test-vlan"}
    client.update.return_value = {"vlan_id": "res-123", "new_name": "test-vlan-updated"}
...
class TestCreateVlan:
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        mock_api_client.create("vlan", params)
        mock_api_client.create.assert_called_once_with("vlan", params)
...
class TestUpdateVlan:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("vlan", "res-123", {"name": "updated"})
        assert result["name"] == "test-vlan-updated"
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("vlan", "bad-id", {})
...
class TestDeleteVlan:
    ...
    def test_delete_nonexistent_is_noop(self, mock_api_client):
        ...
        result = mock_api_client.get("vlan", "missing-id")
    ...
    def test_delete_api_error(self):
        ...
        with pytest.raises(Exception, match="403 Forbidden"):
            client.delete("vlan", "res-123")


class TestGetVlan:
    """Tests for getting a vlan."""
    ...
    def test_get_nonexistent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        result = mock_api_client.get("vlan", "nonexistent")
        assert result is None
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("vlan", "res-123")


class TestListVlan:
    """Tests for listing vlan resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"vlan_id": "1", "name": "first"},
            {"vlan_id": "2", "new_name": "second"},
        ]
        result = mock_api_client.list("vlan")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        result = mock_api_client.list("vlan", filters={"new_name": "match"})
        assert len(result) == 1


class TestIdempotencyVlan:
    """Tests for idempotent behavior of vlan."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("vlan", "res-123")
    ...
    def test_delete_absent_is_idempotent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("vlan", "missing") is not None
        assert exists is False


class TestErrorHandlingVlan:
    """Tests for error handling in vlan."""
