"""Unit tests for stevefulme1.unraid.docker_container module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.unraid.plugins.modules.docker_container"
CLIENT_PATH = "ansible_collections.stevefulme1.unraid.plugins.module_utils.unraid_api"


@pytest.fixture
def mock_api_client():
    """Mock API client for docker_container."""
    client = MagicMock()
...
def existing_resource():
    """Return a dict representing an existing docker_container."""
    return {
        "container_id": "res-123",
...
class TestCreateDockerContainer:
    """Tests for creating a docker_container."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("docker_container", {"name": "test-docker_container"})
        assert result["container_id"] == "res-123"
        assert result["name"] == "test-docker_container"
        mock_api_client.create.assert_called_once()

    def test_create_with_all_params(self, mock_api_client):
        """Verify create passes all parameters to API."""
        params = {
            "name": "full-docker_container",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("docker_container", params)
        mock_api_client.create.assert_called_once_with("docker_container", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("docker_container", {"name": "dup"})

    def test_create_check_mode_no_api_call(self, mock_api_client):
        """Verify check_mode skips actual API call."""
        check_mode = True
        if check_mode:
            result = {"changed": True, "docker_container": {}}
        else:
            result = mock_api_client.create("docker_container", {})
        assert result["changed"] is True
        mock_api_client.create.assert_not_called()


class TestUpdateDockerContainer:
    """Tests for updating a docker_container."""
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("docker_container", "res-123", {"name": "updated"})
        assert result["name"] == "test-docker_container-updated"
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("docker_container", "bad-id", {})


class TestDeleteDockerContainer:
    """Tests for deleting a docker_container."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("docker_container", "res-123")
        mock_api_client.delete.assert_called_once_with("docker_container", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("docker_container", "missing-id")
    ...
    def test_delete_api_error(self):
        ...
        with pytest.raises(Exception, match="403 Forbidden"):
            client.delete("docker_container", "res-123")


class TestGetDockerContainer:
    """Tests for getting a docker_container."""

    def test_get_existing(self, mock_api_client, existing_resource):
        """Verify get returns resource when it exists."""
        mock_api_client.get.return_value = existing_resource
        result = mock_api_client.get("docker_container", "res-123")
        assert result["container_id"] == "res-123"

    def test_get_nonexistent(self, mock_api_client):
        """Verify get returns None for missing resource."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("docker_container", "nonexistent")
        assert result is None
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("docker_container", "res-123")


class TestListDockerContainer:
    """Tests for listing docker_container resources."""
...
class TestIdempotencyDockerContainer:
    """Tests for idempotent behavior of docker_container."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("docker_container", "res-123")
    ...
    def test_delete_absent_is_idempotent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("docker_container", "missing") is not None
        assert exists is False


class TestErrorHandlingDockerContainer:
    """Tests for error handling in docker_container."""
    ...
    def test_auth_failure(self):
        ...
        with pytest.raises(Exception, match="401 Unauthorized"):
            client.create("docker_container", {})
