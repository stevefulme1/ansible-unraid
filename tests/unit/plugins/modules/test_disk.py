"""Unit tests for stevefulme1.unraid.disk module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.unraid.plugins.modules.disk"
CLIENT_PATH = "ansible_collections.stevefulme1.unraid.plugins.module_utils.unraid_api"


@pytest.fixture
def mock_api_client():
    """Mock API client for disk."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"disk_name": "res-123", "new_disk_name": "test-disk"}
    client.update.return_value = {"new_disk_name": "res-123", "disk_name": "test-disk-updated"}
...
def existing_resource():
    """Return a dict representing an existing disk."""
    return {
        "disk_name": "res-123",
        "new_disk_name": "test-disk",
        "state": "active",
    }


class TestCreateDisk:
    """Tests for creating a disk."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("disk", {"disk_name": "test-disk"})
        assert result["disk_name"] == "res-123"
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        mock_api_client.create("disk", params)
        mock_api_client.create.assert_called_once_with("disk", params)
...
class TestUpdateDisk:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("disk", "res-123", {"disk_name": "updated"})
        assert result["disk_name"] == "test-disk-updated"
    ...
    def test_update_idempotent_no_change(self, mock_api_client, existing_resource):
        ...
        desired = {"disk_name": existing_resource["disk_name"]}
        current = {"disk_name": existing_resource["disk_name"]}
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("disk", "bad-id", {})


class TestDeleteDisk:
    """Tests for deleting a disk."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("disk", "res-123")
        mock_api_client.delete.assert_called_once_with("disk", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("disk", "missing-id")
...
class TestGetDisk:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("disk", "res-123")


class TestListDisk:
    """Tests for listing disk resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"disk_name": "1", "new_disk_name": "first"},
            {"new_disk_name": "2", "disk_name": "second"},
        ]
        result = mock_api_client.list("disk")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"disk_name": "1", "new_disk_name": "match"}]
        result = mock_api_client.list("disk", filters={"new_disk_name": "match"})
        assert len(result) == 1


class TestIdempotencyDisk:
    """Tests for idempotent behavior of disk."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("disk", "res-123")
        desired_params = {"disk_name": current["disk_name"]}
        # If resource exists and matches desired state, no change
        changed = desired_params["disk_name"] != current["disk_name"]
        assert changed is False

    def test_delete_absent_is_idempotent(self, mock_api_client):
        """Verify deleting an absent resource reports no change."""
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("disk", "missing") is not None
        assert exists is False


class TestErrorHandlingDisk:
    """Tests for error handling in disk."""
